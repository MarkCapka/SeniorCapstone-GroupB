...
/*/
    ...
    Box boxers2 = createsolar(solarPanelTwo, 39, 3.64, 65, rAX, rAZ, rAY);
        ...
        if (((sliceofday+sunrise)>currentTime) ){
            double angle = 30;
            //double anglem = Math.toRadians(angle);
            sun.setTranslateX(200*Math.cos(angle));
            sun.setTranslateY(200*Math.sin(angle));
            //sceneRoot.getChildren().add(sun);
        }


         if((2*sliceofday + sunrise)>currentTime&&(currentTime>sliceofday+sunrise)){
            double angle = 60;
            double anglem = Math.toRadians(angle);
            sun.setTranslateX(200*Math.cos(angle));
            sun.setTranslateY(200*Math.sin(angle));
