...
public class SkyboxApplication extends Application {
    ...
    boolean box3closest;
    boolean box4closest;
   boolean gbox1closest;
        boolean    gbox2closest;
    int sliceofday;
///hardcoded time for now
    ...
    public double distancecalc(Box box, Group sun){
    ...
    public void colorSetOpt(Box box1, Box box2, Box box3, Box box4,Group sunOb){
        double b1d = calculateLightIntesity(box1,sunOb);
        double b2d = calculateLightIntesity(box2,sunOb);
        double b3d = calculateLightIntesity(box3,sunOb);
        double b4d = calculateLightIntesity(box4,sunOb);


        System.out.println(b1d);
        System.out.println(b2d);
        System.out.println(b3d);
        System.out.println(b4d);
       // int b1d = 1;
        //int b2d = 2;
        //int b3d = 4;
        //int b4d = 5;
       // box1closest = true;
        ...
        //box3closest = false;
        //box4closest = false;
        if((b1d>b2d)&&(b1d>b3d)&&(b1d>b4d)) {

            Color color = Color.RED;
            Color color1 = Color.ORANGE;
            PhongMaterial col = new PhongMaterial(color);
            PhongMaterial col1 = new PhongMaterial(color1);
            box1.setMaterial(col);
            box2.setMaterial(col1);
            box3.setMaterial(col1);
            box4.setMaterial(col1);
            System.out.println("solar panel 1 most light");
        }
         else if ((b2d > b1d) && (b2d > b3d) && (b2d > b4d)) {
                Color color = Color.RED;
                Color color1 = Color.ORANGE;
                PhongMaterial col = new PhongMaterial(color);
                PhongMaterial col1 = new PhongMaterial(color1);
                box2.setMaterial(col);
                box1.setMaterial(col1);
                box3.setMaterial(col1);
                box4.setMaterial(col1);
                System.out.println("solar panel 2 most light");
            }

          else if((b3d > b2d)&&(b3d>b1d)&&(b3d>b4d)){
             Color color = Color.RED;
             Color color1 = Color.ORANGE;
             PhongMaterial col = new PhongMaterial(color);
             PhongMaterial col1 = new PhongMaterial(color1);
             box3.setMaterial(col);
             box1.setMaterial(col1);
             box2.setMaterial(col1);
             box4.setMaterial(col1);
             System.out.println("solar panel 3 most light");

        }  else if((b4d > b1d)&&(b4d>b3d)&&(b4d>b2d)){
            Color color = Color.RED;
            Color color1 = Color.ORANGE;
            PhongMaterial col = new PhongMaterial(color);
            PhongMaterial col1 = new PhongMaterial(color1);
            box4.setMaterial(col);
            box2.setMaterial(col1);
            box3.setMaterial(col1);
            box1.setMaterial(col1);
            System.out.println("solar panel 4 most light");
...
/*/
    public void gcolorSetOpt(Box gbox1, Box gbox2, Group sunOb){
        Double b1d = distancecalc(gbox1, sunOb);
        Double b2d = distancecalc(gbox2, sunOb);
       // Double b3d = distancecalc(box3, sunOb);
        //Double b4d = distancecalc(box4, sunOb);
        //int b1d = 4;
        //int b2d = 1;
        //int b3d = 2;
        //int b4d = 5;
        ...
        if(gbox1closest = true){
            ...
            PhongMaterial col = new PhongMaterial(color);
            PhongMaterial col1 = new PhongMaterial(color1);
            gbox1.setMaterial(col);
            gbox2.setMaterial(col1);
        ...
        if(gbox2closest = true){
            ...
            PhongMaterial col = new PhongMaterial(color);
            PhongMaterial col1 = new PhongMaterial(color1);
            gbox2.setMaterial(col);
            gbox1.setMaterial(col1);
...
/*/
    ...
    public double calculateLightIntesity(Box box, Group Sun){
        double xdis = (Sun.getTranslateX() - box.getTranslateX());
        double ydis = (Sun.getTranslateY() - box.getTranslateY());
...
/*/
    ...
    Box boxers2 = createsolar(solarPanelTwo, 39, 3.64, 65, rAX, rAZ, rAY);
        ...
        //Point3D point1 = new Point3D(boxers.getTranslateX(), boxers.getTranslateY(), boxers.getTranslateZ());
        //Point3D point2 = new Point3D(sun.getTranslateX(), sun.getTranslateY(), sun.getTranslateZ());
       ...
       // System.out.println(distance);
        ...
        if(slice >= currenttime) {
            ...
            sun.setTranslateX(x33 * 1000);
            sun.setTranslateY(-y33 * 1000);
            //sceneRoot.getChildren().add(sun);
         ...
         if(((2*slice)>=currenttime)&&(slice<currenttime)) {
            ...
            sun.setTranslateX(x66 * 1000);
            sun.setTranslateY(-y66 * 1000);
            sun.setTranslateZ(30);
        ...
        if(((4*slice)>=currenttime)&&(3*slice<currenttime)) {
            ...
            sun.setTranslateX(x12 * 1000);
            sun.setTranslateY(-y12 * 1000);
        ...
        if(((3*slice)>=currenttime)&&(2*slice<currenttime)) {
            ...
            sun.setTranslateX(x90 * 1000);
            sun.setTranslateZ(50);
            sun.setTranslateY(-y90 * 1000);
        ...
        if(((5*slice)>=currenttime)&&(4*slice<currenttime)) {
            ...
            sun.setTranslateX(x150 * 1000);
            sun.setTranslateZ(50);
            sun.setTranslateY(-y150 * 1000);
            //sceneRoot.getChildren().add(sun4);
        }
        if(((6*slice)>=currenttime)&&(5*slice<currenttime)) {
            ...
            sun.setTranslateX(x180 * 1000);
            sun.setTranslateZ(50);
            sun.setTranslateY(-y180 * 1000);
            //sceneRoot.getChildren().add(sun4);
        }
        sceneRoot.getChildren().add(sun);
        colorSetOpt(boxers, boxers2, boxers3, boxers4, sun);
        //houseImport.setTranslateX(0);
           // houseImport.setTranslateY(0);
           // houseImport.setTranslateZ(0);
